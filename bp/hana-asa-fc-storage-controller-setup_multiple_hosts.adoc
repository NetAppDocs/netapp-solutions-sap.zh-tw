...
下表顯示 2+1 多主機 SAP HANA 系統的 LUN 組態。

|===
| 目的 | LUN | LVM 邏輯磁碟區 


| 資料主機 1 | FC5_data_mnt00001_1 FC5_data_mnt00001_2 | FC5_data_mnt00001-vol 


| 資料主機 2 | FC5_data_mnt00002_1 FC5_data_mnt00002_2 | FC5_data_mnt00002-vol 


| 記錄主機 1 | FC5_log_mnt00001_1 FC5_log_mnt00001_2 | FC5_log_mnt00001-vol 


| 記錄主機 2 | FC5_log_mnt00002_1 FC5_log_mnt00002_2 | FC5_log_mnt00002-vol 
|===

NOTE: /hana/shared 磁碟區必須可供所有 SAP HANA 主機存取，因此通常使用 NFS 匯出。由於 NetApp ASA 系統不支援 NFS 協定，NetApp 建議為 /hana/shared 檔案系統使用額外的 AFF 或 FAS 系統。
...
NOTE: 如果使用 SAP Local Secure Store 服務（LSS），則 `/lss/shared/SID` 目錄必須可在所有主機上存取，因此通常使用 NFS 匯出。由於 NetApp ASA 系統不支援 NFS 作為通訊協定，NetApp 建議為 /lss/shared 檔案系統使用額外的 AFF 或 FAS 系統。
...
. 提供所需資訊：
+
.. 要與 LVM 合併的日誌 LUN 名稱，例如 `FC5_log_mnt00001`
.. 要與 LVM 組合的 LUN 數量，例如 2
