...
NOTE: 所建立原則中的SnapMirror標籤必須符合SnapCenter 在功能複本原則組態中定義的標籤。如需詳細資料、請參閱「<<Policy for daily Snapshot backups with SnapVault replication>>。」
...
.. 完成資源組態後、請依照「」一節所述、執行資源保護的組態設定<<Resource protection configuration>>。」
