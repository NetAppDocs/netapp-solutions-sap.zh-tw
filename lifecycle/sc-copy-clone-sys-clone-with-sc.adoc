...
+
如果不再需要該系統，則依照下列步驟執行清理程序：

+
.. 執行 SAP HANA 資料磁碟區的 SnapCenter 複製刪除工作流程，包括資料庫關機。
+
[NOTE]
====
SnapCenter 複製刪除工作流程預設不會刪除  `/etc/fstab` 項目。

為了使  `/etc/fstab` 條目在複製刪除過程中自動刪除，必須先恢復複製並將其設定為自動探索資源。如果未執行此步驟，則  `/etc/fstab` 條目不會自動刪除，需要手動移除。

====
.. 停止SAP HANA服務。
.. 針對 SAP HANA 共享磁碟區執行 SnapCenter 複製刪除工作流程。
