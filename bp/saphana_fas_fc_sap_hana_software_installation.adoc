...
link:saphana_fas_fc_i_o_stack_configuration_for_sap_hana.html["先前版本：SAP HANA的I/O堆疊組態。"]
...
....
+
使用SAP「hdbcm」安裝工具、在其中一部工作者主機上執行下列命令、即可開始安裝。使用「addhosts」（附加主機）選項來新增第二位工作者（sapcc-hana）和待命主機（sapcc-hana TST-05）。儲存「global.ini`」檔案的目錄、包含在「sectorage_cfg/」CLI選項（`-storage _cfg//hana / shared'）中。視所使用的作業系統版本而定、在安裝SAP HANA資料庫之前、可能需要先安裝pherton 2.7。
...
link:saphana_fas_fc_adding_additional_data_volume_partitions_for_sap_hana_single-host_systems.html["接下來：為SAP HANA單一主機系統新增額外的資料Volume分割區。"]
