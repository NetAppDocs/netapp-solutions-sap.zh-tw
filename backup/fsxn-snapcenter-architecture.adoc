...
本文檔描述了在 FSx for ONTAP上具有單一租用戶的 SAP HANA MDC 單主機系統最常見的SnapCenter設定選項。對於特定的 SAP HANA 系統，例如多主機系統，也可能存在其他設定選項，並且在某些情況下，這些選項是必要的。有關其他配置選項的詳細說明，請參閱 link:hana-sc-generic-concepts.html["概念與最佳實務做法（netapp.com）SnapCenter"^]。
