...
---
= 利用Libelle SystemCopy自動化SAP系統複製作業
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
儘管修復系統和災難復原測試系統通常使用 SAP 系統克隆（不需要大量後處理作業）來提供，但對於重新整理的測試和訓練系統，必須套用這些後處理步驟以實現與來源系統的共存。因此，本文件著重於 SAP 系統重新整理情境。有關不同使用案例的更多詳細資訊，請參閱技術報告 link:sc-copy-clone-introduction.html["利用SnapCenter 功能實現SAP HANA系統複製與複製作業自動化"^]。
