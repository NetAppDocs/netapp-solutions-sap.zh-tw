...
ONTAP Autonomous Ransomware Protection 所建立的 Snapshot 無法用於 SAP HANA 資料庫的前向還原。因此，不建議將其用於 SAP HANA 資料和記錄磁碟區，並且應停用這些磁碟區的功能。請改用以 SnapCenter 為基礎、具有防竄改 Snapshot 的資料保護來進行勒索軟體防護：link:../backup/hana-sc-generic-scope.html["SAP HANA利用SnapCenter NetApp備份與還原"]。ONTAP Autonomous Ransomware Protection 可能對其他磁碟區（例如 /hana/shared）有益。

雖然可以在建立 ONTAP 叢集磁碟區時停用此功能的自動啟動，但建議保留預設設定，並在不需要此功能的磁碟區上停用此功能。
...
每個 NS224 NVMe 磁碟櫃每個控制器都連接兩個 100GbE 連接埠，如下圖所示。每個機櫃內的磁碟必須平均分配給HA配對的兩個控制器。ADPv2（如Aggregate組態一章所述）也用於NS224磁碟櫃。
...
NOTE: 如果使用 SAP Local Secure Store 服務（LSS）， `/lss/shared/SID` 目錄（LSS 的預設安裝目錄）位於本機磁碟上。在採用磁碟型複寫的災難恢復設定中，NetApp 建議在  `SID_shared` 磁碟區內為  `/lss/shared/SID` 目錄建立一個目錄，以便所有檔案系統都位於中央儲存設備上。
...
NOTE: 如果使用 SAP Local Secure Store 服務（LSS），則  `/lss/shared/SID` 目錄必須可從所有主機存取。NetApp 建議在  `SID_shared` 磁碟區內為  `/lss/shared/SID` 目錄建立一個子目錄。
...
| SID_Shared | lss-shared | /lss/shared/SID | 安裝於所有主機 
...
| 停用勒索軟體保護，但 SID_shared 磁碟區除外 | vol modify -vserver <vserver-name> -volume <volname> -anti-ransomware-state disabled 
...
NOTE: 請注意，停用數字 ID 需要使用者管理，如一節所述link:hana-aff-nfs-install-prep-nfsv4.html["「NFSv4的SAP HANA安裝準備。」"]


NOTE: NFSv4 網域 ID 必須在所有 Linux 伺服器上設為相同的值(`/etc/idmapd.conf`）和 SVM ，如一節所述link:hana-aff-nfs-install-prep-nfsv4.html["「NFSv4的SAP HANA安裝準備。」"]
