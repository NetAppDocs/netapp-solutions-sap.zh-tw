...
所有後續步驟都與從主儲存設備複製的工作流程相同、如「」一節所述<<Cloning from primary storage with tenant name equal to SID>>。」
...
所需的SnapCenter 步驟與本節中所述相同<<Cloning from primary storage with tenant name equal to SID>>。」 不同之處在於指令碼「sc-system-refresh.sh」中的租戶還原作業。
