...
NOTE: 所建立原則中的SnapMirror標籤必須符合SnapCenter 在功能複本原則組態中定義的標籤。如需詳細資訊、請 link:saphana-br-scs-snapcenter-initial-configuration.html#snapshot-policy["每日Snapshot備份與SnapVault 還原複寫的原則"]參閱。
...
.. 完成資源組態後，請依照一節中所述link:saphana-br-scs-snapcenter-resource-specific-configuration-for-sap-hana-database-backups.html#resource-protection["資源保護組態"]，執行資源保護的組態。
