...
NOTE: 以下範例展示了使用 LSS 的 SID=NF2 SAP HANA 資料庫。

若要建立必要的子目錄、請執行下列其中一項動作：

* 對於單主機系統、掛載  `/hana/shared` 磁碟區並建立  `shared`、 `lss-shared` 和  `usr-sap` 子目錄。
...
....
* 對於多主機系統，掛載  `/hana/shared` 磁碟區並為每個主機建立  `shared`、 `lss-shared` 和  `usr-sap` 子目錄。
+
範例命令顯示2+1多主機HANA系統。
...
| /lss/shared | NFSv3或NFSv4 | NFSv3或NFSv4 
...
....
. 執行  `mount -a` 以在所有主機上掛載檔案系統。


下一個範例展示了一個多主機 SAP HANA 資料庫，其 SID=NF2，資料和日誌檔案系統使用 NFSv4.1， `/hana/shared`、 `/lss/shared` 和  `/usr/sap/NF2` 檔案系統使用 NFSv3。讀取操作的 NFS 傳輸大小為 1MB，寫入操作的 NFS 傳輸大小為 256k。
