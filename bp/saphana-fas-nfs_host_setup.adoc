...
`echo options nfs max_session_slots=<calculated value> > /etc/modprobe.d/nfsclient.conf`、然後再重新開機。也必須調整伺服器端值、如所述設定工作階段插槽數link:saphana-fas-nfs_storage_controller_setup.html#nfs-configuration-for-nfsv4["NFSv4的NFS組態。"]
