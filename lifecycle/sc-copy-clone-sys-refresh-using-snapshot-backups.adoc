...
啟用 SAP HANA 持久加密後，根據用於加密根金鑰的安全儲存（執行個體 SFSS 或本機安全儲存 LSS），在目標系統上復原 SAP HANA 資料庫之前，需要執行額外的步驟。

使用 SFSS 時，根金鑰儲存在檔案系統中，除了 HANA 資料庫之外，還必須單獨備份。然後，必須將根金鑰備份匯入目標系統，才能還原 HANA 資料庫。如果根金鑰未發生更改，則只需執行此操作一次。

使用 LSS，根金鑰會與 HANA 資料庫一起備份和還原，只需一次操作即可完成。為了能夠復原 HANA 資料庫，您需要擁有用於備份根金鑰的密碼。
...
NOTE: 您需要使用至少 HANA 版本 2.00.088，否則所述程序將無法運作，另請參閱 SAP note 3613110。

若要還原使用加密和 LSS 的 HANA 系統、您需要用於備份根金鑰的密碼。
...
....
可用於在目標系統的 LSS 設定中設定來源系統的密碼。系統資料庫和租戶資料庫必須使用相同的語句。

如果事先沒有設定密碼，您可以在還原作業期間提供密碼。
