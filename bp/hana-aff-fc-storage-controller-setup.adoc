...
....
建議使用 1000MB/s 作為指定 LUN 的最大處理量。如果應用程式需要更高的處理量，則應使用具有 LUN 分段的多個 LUN 來提供所需的頻寬。本指南在一節中提供以 Linux LVM 為基礎的 SAP HANA 範例link:hana-aff-fc-host-setup.html#create-lvm-volume-groups-and-logical-volumes["主機設定"]。
