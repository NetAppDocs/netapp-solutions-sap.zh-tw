...
SAP主機代理程式也安裝在此主機上。NetApp 提供者指令碼和 Ansible 教戰手冊是在 Ansible 控制節點上設定的、如所述link:lama-ansible-appendix-provider-script-configuration-and-ansible-playbooks.html["「附錄：供應商指令碼組態。」"]
