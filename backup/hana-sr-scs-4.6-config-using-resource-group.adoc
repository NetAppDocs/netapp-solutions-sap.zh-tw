...
已為 HANA 系統資料庫建立了一個名為「SnapCenter」的資料庫用戶，並賦予其執行備份和復原作業所需的權限（請參閱）。 link:hana-sc-generic-scope.html["SAP HANA利用SnapCenter 功能進行備份與還原"^]）。必須在兩台主機上使用上述資料庫使用者設定 HANA 使用者儲存金鑰。
...
SnapCenter 4.6 資源組配置的復原作業與自動發現的非系統複製設定相同。所有恢復和自動恢復選項均可使用。更多詳情請參閱技術報告。 link:hana-sc-generic-scope.html["SAP HANA利用SnapCenter 功能進行備份與還原"^]。
