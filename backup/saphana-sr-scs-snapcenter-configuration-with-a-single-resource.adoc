...
link:saphana-sr-scs-snapcenter-4.6-configuration-using-a-resource-group.html["上一篇：SnapCenter 使用資源群組進行的功能組態設定。"]
...
link:saphana-sr-scs-restore-and-recovery-from-a-backup-created-at-the-other-host.html["下一步：從在其他主機上建立的備份進行還原與還原。"]
