...
. 按照"<<Aggregate configuration>>。」
. 按照"<<SVM configuration>>。」
. 按照"<<LIF configuration>>。」
...
. 按照"<<Volume options>>。」
. 按照"<<NFS configuration for NFSv3>>"或NFSv4（如中所述）。<<NFS configuration for NFSv4>>。」
. 將磁碟區掛載到命名空間、並依照「<<Mount volumes to namespace and set export policies>>。」
