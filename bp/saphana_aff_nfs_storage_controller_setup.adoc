...
link:saphana_aff_nfs_time_synchronization.html["先前：時間同步。"]
...
. 按照" configuration。」
. 按照" configuration。」
. 在集合體內建立磁碟區、如「 configuration for SAP HANA single host systems"和" configuration for SAP HANA multiple host systems。」
. 按照" options。」
. 按照" configuration for NFSv3"或NFSv4（如中所述）。 configuration for NFSv4。」
. 將磁碟區掛載到命名空間、並依照「 volumes to namespace and set export policies。」
...
如果使用SAP HANA多個主機系統、請在SVM上設定NFSv4租用時間（如下表所示）。
...
|===
從HANA 2.0 SPS4開始、HANA提供可控制容錯移轉行為的參數。NetApp建議使用這些HANA參數、而非在SVM層級設定租用時間。

參數位於nameserver.ini`範圍內、如下表所示。請在這些區段內保留預設的重試時間間隔10秒。
...
link:saphana_aff_nfs_host_setup.html["下一步：主機設定。"]
