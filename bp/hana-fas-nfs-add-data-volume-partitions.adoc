...
除了一節中所述的工作之外，您還link:hana-fas-nfs-host-setup.html["「主機設定、」"]必須為新的額外資料磁碟區建立額外的裝載點和 Fstab 項目，而且您必須裝載新的磁碟區。
...
....
image:saphana-fas-nfs_image19.png["此圖顯示輸入 / 輸出對話方塊或表示寫入內容"]
