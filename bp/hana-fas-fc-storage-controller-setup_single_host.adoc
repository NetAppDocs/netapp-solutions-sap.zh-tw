...
NOTE: 如果使用 SAP Local Secure Store 服務（LSS），則 `/lss/shared/SID` 目錄（LSS 的預設安裝目錄）位於本地磁碟。在使用磁碟型複寫的災難復原架構中，NetApp 建議在 `SID1_shared` 磁區內為 `/lss/shared/SID` 目錄建立額外的 LUN，以便所有檔案系統都位於中央儲存設備上。
