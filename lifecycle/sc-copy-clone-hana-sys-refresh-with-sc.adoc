...
|  | SAP HANA 持續性加密 | SFSS：初始狀態下，或如果來源系統的根金鑰已更改，則必須先在目標系統匯入根金鑰備份，然後才能執行復原作業。LSS：初始狀態下，或如果根金鑰備份密碼已更改，則必須在 LSS 設定中設定密碼。 
...
|  | SAP HANA 持續性加密 | SFSS：如果來源系統的 DBID 在目標系統中不存在，則必須先還原系統資料庫，然後才能匯入該租用戶的根金鑰備份。LSS：在租用戶資料庫復原期間，LSS 拓樸將自動與 HANA 拓樸同步。 
...
所有 SAP HANA 系統都必須依照設定指南進行設定。 link:../bp/hana-aff-nfs-introduction.html["SAP HANA on NetApp AFF 的NFS系統"]。SnapCenter和 SAP HANA 資源均依照最佳實務指南進行設定。 link:../backup/hana-sc-generic-scope.html["SAP HANA利用SnapCenter 功能進行備份與還原"]。
...
. 安裝 SAP HANA 目標系統
. 依照SnapCenter中的描述配置 SAP HANA 系統 link:../backup/hana-sc-generic-scope.html["SAP HANA利用SnapCenter 功能進行備份與還原"]
+
...
在 Fibre Channel SAN 設定中，您需要在下一個畫面中提供所使用的傳輸協定。
