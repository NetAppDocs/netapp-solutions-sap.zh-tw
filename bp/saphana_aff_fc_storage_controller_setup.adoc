...
link:saphana_aff_fc_time_synchronization.html["先前：時間同步。"]
...
. 檢查正確的磁碟櫃組態、如「 shelf connection。」
. 建立及設定所需的集合體、如「 configuration。」
. 創建儲存虛擬機（SVM）、如「 virtual machine configuration。」
. 創建邏輯介面（l生命）、如「 interface configuration。」
. 建立連接埠集、如「 port sets。」
. 在集合體中建立啟動器群組、磁碟區和LUN、如建立" and volumes and mapping LUNs to initiator groups。」
...
image:saphana_aff_fc_image10.png["錯誤：缺少圖形影像"]
...
image:saphana_aff_fc_image11.png["錯誤：缺少圖形影像"]
...
image:saphana_aff_fc_image12.jpg["錯誤：缺少圖形影像"]
...
image:saphana_aff_fc_image13.jpeg["錯誤：缺少圖形影像"]

在使用ONTAP NetApp 9.8 System Manager建立SVM期間、您可以選取所有必要的實體FCP連接埠、而且每個實體連接埠會自動建立一個LIF。

image:saphana_aff_fc_image14.jpeg["錯誤：缺少圖形影像"]
...
image:saphana_aff_fc_image15.jpeg["錯誤：缺少圖形影像"]
...
image:saphana_aff_fc_image16.jpg["錯誤：缺少圖形影像"]
...
image:saphana_aff_fc_image17.jpg["錯誤：缺少圖形影像"]
...
+
image:saphana_aff_fc_image18.jpeg["錯誤：缺少圖形影像"]

. 確認已成功配置儲存設備。


image:saphana_aff_fc_image19.jpeg["錯誤：缺少圖形影像"]
...
link:saphana_aff_fc_sap_hana_storage_connector_api.html["下一步：SAP HANA儲存連接器API。"]
