...
image:saphana-fas-nfs_image2.png["連接到儲存高可用性 (HA) 對的八個 SAP HANA 主機"]
...
image:saphana-fas-nfs_image4.png["將額外的 SAP HANA 主機移至現有的儲存控制器 HA 配對"]

SAP HANA環境獨立於已部署FAS 的整個系統、也可藉由新增任何認證儲存控制器來擴充、以符合所需的節點密度（下圖）。

image:saphana-fas-nfs_image5a.png["新增額外的 HA 配對"]
...
有關使用SnapCenter的 SAP HANA 備份和復原解決方案的詳細信息，請參閱 link:../backup/hana-sc-generic-scope.html["SAP HANA利用SnapCenter 功能進行備份與還原"^]。
