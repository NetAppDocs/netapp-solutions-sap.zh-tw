...
[role="lead"]
本節提供如何在 SUSE KVM 中使用NetApp儲存為 SAP HANA 配置 SR-IOV 的資訊。



== SR-IOV的設定

本部分介紹如何使用 Mellanox Connect-X 網路卡設定 SR-IOV。它是基於 https://enterprise-support.nvidia.com/s/article/HowTo-Configure-SR-IOV-for-ConnectX-4-ConnectX-5-ConnectX-6-with-KVM-Ethernet["NVIDIA企業支援入口網站 | 如何為 ConnectX-4/ConnectX-5/ConnectX-6 設定 SR-IOV（乙太網路）"]。  SUSE SAP HANA KVM 指南以英特爾網卡為例對此進行了描述。 + 建議使用冗餘乙太網路連接，將兩個實體連接埠組合成 trunk/bond。指派給虛擬機器的虛擬連接埠（VF）也需要在虛擬機器內部進行中繼配置。
...
在本章中，SR-IOV 功能已啟用，每個實體連接埠的最大 VF（虛擬介面）數量已在適配器韌體中設置，並且每個實體連接埠（100GbE）使用的 VF 數量已設置，在本例中為 4。


NOTE: 分配給虛擬機器的每個虛擬功能至少需要 10Gbit/s 的頻寬。例如，不要為 25GbE 實體連接埠建立和指派兩個以上的 VF。
...
NOTE: 如果您已經使用這兩個連接埠配置了綁定，則需要執行第一個命令來建立綁定：
...
上述配置在重啟後會失效。  + 為了使其持久化，需要建立一個名為 /etc/systemd/system/after.local 的 systemd 單元文件，內容如下：
...
....
之後，建立腳本 /etc/init.d/after.local：
...
本章介紹如何使用 _virt-manager_ 將虛擬介面指派給虛擬機器。

. 啟動 virt-manager。+ image:kvm-hana-image2.png["寬度=601，高度=385"]
. 開啟所需的虛擬機器。+ image:kvm-hana-image3.png["寬度=300，高度=385"]
. 按下“添加硬體”。+ image:kvm-hana-image4.png["Virtmanager 增加硬件，寬度=601，高度=443"]
. 從 PCI 主機設備清單中的第一個實體連接埠選擇所需的虛擬網路卡，然後按完成鍵。 + 在此範例中，0000.AB:00:2 - 0000.AB:00:4 屬於第一個實體端口，0000.AB:01:2 - 0000.AB:01:4 屬於第二個實體連接埠。+ image:kvm-hana-image5.png["寬度=601，高度=442"]
. 從 PCI 主機設備清單中選擇下一個虛擬網卡端口，使用第二個實體端口上的虛擬端口，然後按完成。+ image:kvm-hana-image6.png["寬度=601，高度=441"]
...
登入虛擬機器並將兩個虛擬功能配置為綁定。選擇模式 0 或模式 2。請勿使用 LACP，因為 LACP 只能用於實體連接埠。下圖顯示了使用 YAST 的模式 2 配置。  + +image:kvm-hana-image8.png["寬度=601，高度=335"]
