...
該 `/hana/shared`磁碟區必須可供所有 HANA 主機訪問，因此使用 NFS 匯出。儘管 `/hana/shared`檔案系統沒有特定的效能 KPI，但 NetApp 建議使用 10Gb 乙太網路連線。
...
NOTE: 如果使用 SAP Local Secure Store 服務（LSS），則 `/lss/shared/SID`目錄必須可在所有主機上存取。NetApp 建議為 `/lss/shared/SID`目錄建立一個磁碟區，並使用 NFS 將其掛載到所有主機上。
