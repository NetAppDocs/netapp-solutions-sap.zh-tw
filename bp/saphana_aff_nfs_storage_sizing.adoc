...
link:saphana_aff_nfs_architecture.html["上一篇：架構。"]
...
第一個機櫃最少可支援12個NVMe SSD、最多可支援16個SAP HANA主機。完整配置的機櫃（24個SSD）最多可支援48台SAP HANA主機。同樣適用於AFF 內部磁碟的Same A800系統。
...
link:saphana_aff_nfs_infrastructure_setup_and_configuration_overview.html["下一步：基礎架構設定與組態。"]
