...
link:saphana-dr-anf_create_new_volumes_based_on_snapshot_backups_at_the_disaster_recovery_site.html["先前版本：根據災難恢復站台的快照備份建立新的磁碟區。"]
...
....
link:saphana-dr-anf_hana_database_recovery.html["下一步：Hana資料庫恢復。"]
