...
image::saphana-fas-nfs_image13.png[saphana FAS NFS 影像 13.]

使用SSD時、最多可將一個磁碟櫃連接至一個SAS堆疊、以提供SAP HANA主機所需的效能、如下圖所示。每個機櫃內的磁碟必須平均分配給HA配對的兩個控制器。使用DS224C磁碟櫃時、也可以使用四路徑SAS纜線、但不需要。

image::saphana-fas-nfs_image14.png[saphana FAS NFS 影像 14.]
...
image::saphana-fas-nfs_image15.png[saphana FAS NFS 影像 15.]
...
image::saphana-fas-nfs_image16.jpg[saphana FAS NFS 影像 16.]
...
image::saphana-fas-nfs_image17.jpg[saphana FAS NFS 影像 17.]
...
image::saphana-fas-nfs_image18.jpg[saphana FAS NFS 影像 18.]
