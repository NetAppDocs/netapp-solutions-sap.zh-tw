...
有關這些不同的SnapCenter代理和 SAP HANA 資料庫插件安裝選項的更多詳細信息，請參閱技術報告。 link:../backup/hana-sc-generic-scope.html["SAP HANA利用SnapCenter NetApp備份與還原"^]。

以下各節將強調使用SnapCenter 標準安裝或中央通訊主機、將LSC與支援功能整合的差異。值得注意的是、無論安裝選項和使用的資料庫為何、未反白顯示的所有組態步驟都是相同的。

為了從來源資料庫執行基於 Snapshot 複本的自動備份並為新的目標資料庫建立複製，LSC 和 SnapCenter 之間描述的整合使用了 link:sc-copy-clone-introduction.html["利用SnapCenter 功能實現SAP HANA系統複製與複製作業自動化"^] 中所述的設定選項和腳本。
...
* link:sc-copy-clone-introduction.html["利用SnapCenter 功能實現SAP HANA系統複製與複製作業自動化"^]
