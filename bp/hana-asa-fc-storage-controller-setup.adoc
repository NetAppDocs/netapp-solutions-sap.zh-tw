...
....
建議使用 1000MB/s 作為給定 LUN 的最大吞吐量。如果應用程式需要更大的吞吐量，則應使用具有 LUN 條帶化的多個 LUN 來提供所需的頻寬。本指南提供了基於 Linux LVM 的 SAP HANA 範例，請參閱第link:hana-asa-fc-host-setup.html["主機設定"]。
