...
summary: 本節說明如何設定系統、以便在單一主機和多主機系統上安裝SAP HANA軟體。 
---
= SAP HANA軟體安裝
:hardbreaks:
:allow-uri-read: 
...
link:saphana_aff_nfs_sap_hana_data_volume_size.html["先前版本：SAP HANA資料Volume大小。"]
...
. 使用SAP「hdbclm」安裝工具、在其中一部工作者主機上執行下列命令、即可開始安裝。使用「addhosts」選項來新增第二名員工（「sAPCA-Hana TST-07」）和待命主機（「sAPCA-Hana TST-08」）。
+
....
...
link:saphana_aff_nfs_adding_additional_data_volume_partitions.html["下一步：新增額外的資料Volume分割區。"]
